...
export const NotificationHelpers = {
  ...
  async sendMatchInvitationNotifications(
    ...
    try {
      ...
      const invitedPlayerIds = playerIds.filter(
        (id) => id && id !== createdById,
      );
  ...
  async sendMatchConfirmationNotifications(
    ...
    try {
      ...
      const playersNeedingConfirmation = playerIds.filter(
        (id) => id && id !== createdById,
      );

      if (playersNeedingConfirmation.length === 0) {
        console.log(
          "No players need confirmation notifications (creator auto-confirms)",
        );
        return;
      }
  ...
  async sendMatchScoreConfirmedNotification(
    ...
    try {
      ...
      console.error(
        "Failed to send match score confirmed notification:",
        error,
      );
    }
  },
