...
const TeamSelectionModal: React.FC<TeamSelectionModalProps> = ({
  ...
  const renderTeam = (teamNumber: 1 | 2) => {
    ...
    return (
      <View className={`flex-1 p-4 rounded-xl bg-${teamColor}-50 dark:bg-${teamColor}-900/20 mx-2`}>
        ...
        <View className="mb-4">
            ...
            <View key={player.id} className="flex-row items-center mb-2">
              ...
              <View className="ml-3 flex-1">
                ...
                <Text className="text-xs text-gray-500 dark:text-gray-400">
                  {player.glicko_rating ? Math.round(parseFloat(player.glicko_rating)) : 1500}
                </Text>
              </View>
            </View>
          ))}
          ...
          {/* Empty slots */}
          {Array.from({ length: 2 - (players?.length || 0) }).map((_, index) => (
            <View key={index} className="flex-row items-center mb-2 opacity-50">
              <View className={`w-8 h-8 rounded-full border-2 border-dashed border-${teamColor}-300 items-center justify-center`}>
                <Text className={`text-${teamColor}-400`}>?</Text>
              </View>
              <Text className="ml-3 text-gray-500 dark:text-gray-400">Open slot</Text>
            </View>
          ))}
        </View>
        ...
        <Button
          ...
          style={{
            backgroundColor: hasSpace ? (teamNumber === 1 ? '#3B82F6' : '#8B5CF6') : undefined
          }}
        >
          {loading ? (
            <ActivityIndicator size="small" color="#fff" />
          ) : (
            <Text className={hasSpace ? "text-white" : "text-gray-500 dark:text-gray-400"}>
            ...
            {!team1HasSpace && !team2HasSpace && (
              <View className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                <Text className="text-sm text-red-600 dark:text-red-400 text-center">
                  This match is currently full. Check back later for cancellations.
                </Text>
              </View>
            )}
          </View>
...
export default function CleanBrowsePublicMatches() {
  ...
  const [teamSelectionMatch, setTeamSelectionMatch] = useState<PublicMatch | null>(null);
  ...
  const processMatchData = useCallback((matches: any[]): PublicMatch[] => {
    ...
    return matches.map((match) => {
      ...
      const totalAvailableSlots = 4 - team1Players.length - team2Players.length;
      ...
      const canJoin = !isUserInMatch && totalAvailableSlots > 0 && timeUntilMatch > 0;
  ...
  const filteredMatches = useMemo(() => {
    ...
    if (filters.availableSlotsOnly) {
      filtered = filtered.filter(match => 
        match.totalAvailableSlots && match.totalAvailableSlots > 0
      );
    }
  ...
  const handleMatchPress = useCallback((match: PublicMatch) => {
    ...
    if (!match.canJoin) {
      Alert.alert("Cannot Join", "This match is full or has already started.");
      return;
    }
  ...
  const handleTeamSelection = useCallback(async (teamNumber: 1 | 2) => {
    ...
    try {
      ...
      Alert.alert("Join Failed", "Could not join the match. The position may have been filled.");
      fetchPublicMatches();
    } finally {
      setJoining(null);
    }
  ...
  const renderMatchCard = useCallback((match: PublicMatch) => {
    const startTime = new Date(match.start_time);
    ...
    const timeUntilText = match.timeUntilMatch && match.timeUntilMatch > 0
      ...
      >
        ...
        <View className="flex-row items-center justify-between mb-4">
          ...
          <View className="items-end">
            <View className="flex-row items-center">
              <Ionicons name="people" size={16} color="#6B7280" style={{ marginRight: 4 }} />
            ...
            {(match.totalAvailableSlots || 0) > 0 && (
              <Text className="text-xs text-green-600 dark:text-green-400">
                {match.totalAvailableSlots} spot{match.totalAvailableSlots !== 1 ? 's' : ''} open
              </Text>
            )}
          </View>
        </View>
        ...
        <View className="flex-row justify-between mb-4">
          ...
          <View className="flex-1 mr-2">
            <Text className="text-xs text-blue-600 font-medium mb-2">Team 1</Text>
            ...
            {Array.from({ length: 2 - (match.team1Players?.length || 0) }).map((_, index) => (
              <View key={index} className="flex-row items-center mb-1 opacity-50">
          ...
          <View className="flex-1 ml-2">
            <Text className="text-xs text-purple-600 font-medium mb-2">Team 2</Text>
            ...
            {Array.from({ length: 2 - (match.team2Players?.length || 0) }).map((_, index) => (
              <View key={index} className="flex-row items-center mb-1 opacity-50">
        ...
        {/* Action Button */}
        <Button
          variant={match.canJoin && !match.isUserInMatch ? "default" : "outline"}
          className="w-full"
          onPress={() => handleMatchPress(match)}
          disabled={joining === match.id || (!match.canJoin && !match.isUserInMatch)}
        >
          {joining === match.id ? (
            <ActivityIndicator size="small" color="#fff" />
          ) : (
            <Text className={`font-medium ${
              match.canJoin && !match.isUserInMatch ? "text-white" : "text-gray-600 dark:text-gray-400"
            }`}>
              {getActionText()}
            </Text>
          )}
    ...
    return (
      <SafeAreaView className="flex-1 bg-background">
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="mt-4 text-gray-500 dark:text-gray-400">Loading public matches...</Text>
        </View>
      </SafeAreaView>
    );
  }
        ...
        {filteredMatches.length > 0 ? (
          ...
          <View className="items-center justify-center py-16">
            ...
            >
              ...
              <Text className="text-white font-medium">Create Public Match</Text>
            </Button>
          </View>
        )}
      </ScrollView>
