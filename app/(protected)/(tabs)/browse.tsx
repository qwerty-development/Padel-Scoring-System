...
const TeamSelectionModal: React.FC<TeamSelectionModalProps> = ({
  ...
  const team1Average = match.team1Players && match.team1Players.length > 0
    ? match.team1Players.reduce((sum, p) => sum + parseFloat(p.glicko_rating || '1500'), 0) / match.team1Players.length
    : 0;

  const team2Average = match.team2Players && match.team2Players.length > 0
    ? match.team2Players.reduce((sum, p) => sum + parseFloat(p.glicko_rating || '1500'), 0) / match.team2Players.length
    : 0;

  const renderTeamSection = (teamNumber: 1 | 2) => {
    const players = teamNumber === 1 ? match.team1Players : match.team2Players;
    const availableSlots = teamNumber === 1 ? match.team1Slots : match.team2Slots;
    const canJoin = teamNumber === 1 ? match.canJoinTeam1 : match.canJoinTeam2;
    const teamAverage = teamNumber === 1 ? team1Average : team2Average;

    return (
      <View className={`flex-1 p-4 rounded-lg ${teamNumber === 1 ? 'bg-primary/5' : 'bg-indigo-500/5'} mx-2`}>
        <View className="flex-row items-center justify-between mb-3">
          <Text className={`font-bold text-lg ${teamNumber === 1 ? 'text-primary' : 'text-indigo-600'}`}>
            Team {teamNumber}
          </Text>
        ...
        <Button
          ...
          style={{
            backgroundColor: canJoin ? (teamNumber === 1 ? '#1a7ebd' : '#6366f1') : undefined
          }}
        >
          {loading ? (
            <ActivityIndicator size="small" color="#fff" />
          ) : (
            <Text className={`text-sm ${canJoin ? 'text-white' : 'text-muted-foreground'}`}>
        ...
        <View className="bg-background rounded-xl" style={{
          ...
          <View className="p-4 bg-muted/20">
            ...
            {(match.region || match.court) && (
              <Text className="text-sm text-muted-foreground">
                {match.court && match.region ? `${match.court}, ${match.region}` : 
...
export default function ProductionBrowsePublicMatches() {
  ...
  const processMatchData = useCallback((matches: any[]): PublicMatch[] => {
    ...
    return matches.map(match => {
      ...
      const isUserInMatch = isUserPlayer1 || isUserPlayer2 || isUserPlayer3 || isUserPlayer4;
      const userTeam: 1 | 2 | null = 
        (isUserPlayer1 || isUserPlayer2) ? 1 :
        (isUserPlayer3 || isUserPlayer4) ? 2 : null;
      
      // **JOIN CAPABILITY ASSESSMENT**
      const canJoinTeam1 = !isUserInMatch && team1Slots > 0 && timeUntilMatch > 0;
      const canJoinTeam2 = !isUserInMatch && team2Slots > 0 && timeUntilMatch > 0;
  ...
  const filteredMatches = useMemo(() => {
    ...
    if (filters.skillLevel !== 'all') {
      filtered = filtered.filter(match => 
        match.skillLevel?.toLowerCase() === filters.skillLevel
      );
    }
    ...
    filtered.sort((a, b) => {
      ...
      switch (filters.sortBy) {
        ...
        case 'skill':
          ...
          break;
        case 'slots':
          comparison = (b.totalAvailableSlots || 0) - (a.totalAvailableSlots || 0);
          break;
        case 'location':
          ...
          {['today', 'tomorrow', 'week'].map((range) => (
            ...
            >
              <Text className={`text-xs capitalize ${
                filters.timeRange === range ? 'text-primary-foreground' : 'text-foreground'
              }`}>
                {range}
              </Text>
            </TouchableOpacity>
      ...
      <View className="flex-row justify-between items-center">
        ...
        <View className="flex-row gap-2">
          {[
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (filters.sortBy === key) {
                  ...
                  setFilters(prev => ({ ...prev, sortBy: key as any, sortOrder: 'asc' }));
                }
              }}
            ...
            >
              <Text className={`text-xs ${
                filters.sortBy === key ? 'text-primary-foreground' : 'text-foreground'
              }`}>
                {label}
              </Text>
              {filters.sortBy === key && (
      ...
      >
        ...
        <View className="p-4">
          ...
          <Button
            ...
            className="w-full"
            onPress={() => handleMatchPress(match)}
            disabled={joining === match.id || (!match.canJoinTeam1 && !match.canJoinTeam2 && !match.isUserInMatch)}
          >
            {joining === match.id ? (
              <ActivityIndicator size="small" color="#fff" />
            ) : (
              <Text className={`text-sm ${
                getActionButtonVariant() === 'outline' ? "text-primary" : "text-primary-foreground"
              }`}>
                {getActionButtonText()}
              </Text>
            )}
