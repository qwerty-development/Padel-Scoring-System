...
// FIXED: Enhanced Image Processing with Better URI Handling
const processImage = async (uri: string): Promise<string> => {
  try {
    console.log('📷 FIXED: Processing image:', uri);
    ...
    console.log('📷 FIXED: Source file info:', {
    ...
    const processedImage = await ImageManipulator.manipulateAsync(
      ...
      [{ resize: { width: 400, height: 400 } }],
      {
        ...
        base64: false, // Critical: Keep as false for file-based approach
      }
    );
    
    // Verify the processed image exists
    const processedFileInfo = await FileSystem.getInfoAsync(processedImage.uri);
    console.log('📷 FIXED: Processed file info:', {
      exists: processedFileInfo.exists,
      size: processedFileInfo.size,
      uri: processedFileInfo.uri
    });
    
    if (!processedFileInfo.exists || processedFileInfo.size === 0) {
      throw new Error('Image processing resulted in empty or non-existent file');
    }
    
    console.log('📷 FIXED: Image processed successfully');
    return processedImage.uri;
  } catch (error) {
    console.error('📷 FIXED: Image processing error:', error);
    throw new Error(`Image processing failed: ${error.message}`);
  }
};

// COMPLETELY REWRITTEN: File to Blob Conversion using FileSystem
const fileUriToBlob = async (uri: string): Promise<Blob> => {
  try {
    console.log('📷 FIXED: Converting file URI to blob:', uri);
    
    // Method 1: Use FileSystem to read as base64, then convert to Blob
    ...
    console.log('📷 FIXED: Base64 data length:', base64Data.length);
    ...
    console.log('📷 FIXED: Blob created successfully:', {
      size: blob.size,
      type: blob.type
    });
    ...
    console.error('📷 FIXED: File URI to blob conversion error:', error);
    throw new Error(`Failed to convert file to blob: ${error.message}`);
  }
};

// ALTERNATIVE METHOD: FormData Approach (More reliable for React Native)
const uploadUsingFormData = async (
  ...
  try {
    console.log('📷 FIXED: Using FormData upload method');
    ...
    console.log('📷 FIXED: File info for upload:', {
      exists: fileInfo.exists,
      size: fileInfo.size,
      uri: fileInfo.uri
    });
    ...
    // Upload using Supabase REST API with FormData
    ...
    if (previousAvatarUrl && previousAvatarUrl !== publicUrlData.publicUrl) {
      try {
        ...
        if (match && match[1]) {
          ...
          console.log('📷 FIXED: Previous avatar cleaned up:', oldFilePath);
        }
      } catch (cleanupError) {
        console.warn('📷 FIXED: Previous avatar cleanup failed:', cleanupError);
      }
    }
    
    console.log('📷 FIXED: FormData upload completed:', {
      filePath,
    ...
    console.error('📷 FIXED: FormData upload error:', error);
    return {
      success: false,
      error: error.message || 'FormData upload failed'
    };
  }
};

// FIXED: Primary Upload Function with Multiple Fallback Methods
const uploadAvatarComplete = async (
  ...
  try {
    console.log('📷 FIXED: Starting upload with multiple methods');
    
    if (!imageUri || !userId) {
      return { success: false, error: 'Invalid parameters: imageUri and userId are required' };
    }

    // Method 1: Try FormData approach (most reliable for React Native)
    console.log('📷 FIXED: Attempting FormData upload method');
    const formDataResult = await uploadUsingFormData(imageUri, userId, previousAvatarUrl);
    
    if (formDataResult.success) {
      console.log('📷 FIXED: FormData upload successful');
      return formDataResult;
    }
    
    console.log('📷 FIXED: FormData failed, trying Blob method:', formDataResult.error);
    
    // Method 2: Fallback to Blob method with improved file handling
    try {
      ...
      console.log('📷 FIXED: Uploading blob to path:', filePath, 'Size:', imageBlob.size);
      ...
      console.log('📷 FIXED: Blob upload completed successfully');
      return {
        success: true,
        publicUrl: publicUrlData.publicUrl,
        filePath: filePath
      ...
      console.error('📷 FIXED: Blob upload also failed:', blobError);
      return {
        success: false,
        error: `All upload methods failed. Last error: ${blobError.message}`
      };
    }

  } catch (error) {
    console.error('📷 FIXED: Complete upload process error:', error);
    return { success: false, error: `Avatar upload failed: ${error.message}` };
  }
};
...
const captureFromCamera = async () => {
  try {
    ...
    console.error('📷 FIXED: Camera capture error:', error);
    return { success: false, error: `Camera capture failed: ${error.message}` };
  }
};
...
const selectFromLibrary = async () => {
  try {
    ...
    console.error('📷 FIXED: Photo library selection error:', error);
    return { success: false, error: `Photo selection failed: ${error.message}` };
  }
};
...
// FIXED: Avatar deletion (unchanged - working correctly)
const deleteAvatarComplete = async (publicUrl: string) => {
  try {
    ...
    console.log('📷 FIXED: Deleting avatar at path:', filePath);
    ...
    if (deleteError) {
      console.error('📷 FIXED: Deletion error:', deleteError);
      return { success: false, error: `Deletion failed: ${deleteError.message}` };
    }

    console.log('📷 FIXED: Avatar deleted successfully:', filePath);
    return { success: true };

  } catch (error) {
    console.error('📷 FIXED: Deletion error:', error);
    return { success: false, error: `Avatar deletion failed: ${error.message}` };
  }
};
...
export default function Profile() {
  ...
  useEffect(() => {
    if (avatarState.error || avatarState.success) {
      const timer = setTimeout(() => {
        setAvatarState(prev => ({ ...prev, error: null, success: null }));
      }, 5000);
      return () => clearTimeout(timer);
    }
  }, [avatarState.error, avatarState.success]);

  // FIXED: Avatar Upload Handler with Enhanced Error Handling
  const handleAvatarUpload = async () => {
    if (!profile?.id) {
      setAvatarState(prev => ({ ...prev, error: 'User profile not available' }));
      return;
    }

    try {
      console.log('🚀 FIXED: Starting enhanced avatar upload workflow');
      
      setAvatarState({ uploading: false, deleting: false, error: null, success: null });

      const selectionResult = await showImageSelectionOptions();
      
      if (!selectionResult.success) {
        if (!selectionResult.cancelled) {
          setAvatarState(prev => ({ ...prev, error: selectionResult.error || 'Failed to select image' }));
        }
        return;
      }

      if (!selectionResult.uri) {
        setAvatarState(prev => ({ ...prev, error: 'No image selected' }));
        return;
      }

      setAvatarState(prev => ({ ...prev, uploading: true }));
      console.log('🔄 FIXED: Starting upload with improved methods');
      ...
      console.log('✅ FIXED: Avatar upload completed successfully');

    } catch (error) {
      console.error('❌ FIXED: Avatar upload error:', error);
      setAvatarState({
        uploading: false,
        deleting: false,
  ...
  // FIXED: Avatar removal handler (unchanged - working correctly)
  const handleAvatarRemove = async () => {
    if (!profile?.id || !profile.avatar_url) {
      setAvatarState(prev => ({ ...prev, error: 'No profile picture to remove' }));
      return;
    }
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              setAvatarState(prev => ({ ...prev, deleting: true, error: null }));
              console.log('🗑️ FIXED: Starting avatar removal process');

              const deleteResult = await deleteAvatarComplete(profile.avatar_url);
              
              if (!deleteResult.success) {
                console.warn('⚠️ FIXED: Storage deletion failed:', deleteResult.error);
              }
              ...
              console.log('✅ FIXED: Avatar removal completed');

            } catch (error) {
              console.error('❌ FIXED: Avatar removal error:', error);
              setAvatarState({
                uploading: false,
                deleting: false,
    ...
    try {
      ...
      if (matchData) {
        const chronologicalMatches = matchData
          .filter(match => match.team1_score_set1 !== null && match.team2_score_set1 !== null)
          .sort((a, b) => {
  ...
  const handleSignOut = async () => {
    Alert.alert("Sign Out", "Are you sure you want to sign out?", [
      { text: "Cancel", style: "cancel" },
      { text: "Sign Out", onPress: async () => { await signOut(); }, style: "destructive" }
    ]);
  };
    ...
    <View className="items-center mb-4">
          ...
          <View className="w-32 h-32 rounded-full bg-primary items-center justify-center overflow-hidden">
            {profile?.avatar_url && !avatarState.uploading && !avatarState.deleting ? (
              <Image
                ...
                transition={200}
                onError={() => {
                  console.log('🖼️ FIXED: Avatar image failed to load, showing fallback');
                }}
              />
            ) : (
      ...
      {avatarState.error && (
        <View className="mt-2 p-3 bg-red-50 dark:bg-red-900/30 rounded-lg border border-red-200 dark:border-red-800 max-w-xs">
          ...
          <TouchableOpacity 
            onPress={() => setAvatarState(prev => ({ ...prev, error: null }))}
        ...
        <View className="px-6 pb-8 pt-6">
          ...
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-3">
              ...
              <TouchableOpacity onPress={() => router.push('/(protected)/(screens)/edit-profile')}>
