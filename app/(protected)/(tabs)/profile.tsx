...
const safeBase64Decode = (base64String: string): Uint8Array | null => {
  try {
    ...
    if (byteCharacters.length > maxSize) {
      console.error(
        "🚨 PRODUCTION: Decoded data too large:",
...
const selectImageWithProductionSafety = async (
  useCamera: boolean,
): Promise<{
  success: boolean;
  base64?: string;
  ...
  try {
    ...
    const hasPermission = await requestPermissionSafely(
  ...
  try {
    ...
    while (uploadAttempts < maxAttempts) {
      try {
        uploadAttempts++;
        console.log(
          `🔧 PRODUCTION: Upload attempt ${uploadAttempts}/${maxAttempts}`,
        );
        ...
        if (error) {
          ...
          console.warn(
            `🚨 PRODUCTION: Upload attempt ${uploadAttempts} failed:`,
            error,
          );
        ...
        console.log(
          "✅ PRODUCTION: Upload successful on attempt",
          uploadAttempts,
        );
        break;
      } catch (attemptError) {
        lastError =
          attemptError instanceof Error
            ...
            : "Upload failed";
        console.error(
          `🚨 PRODUCTION: Upload attempt ${uploadAttempts} error:`,
          attemptError,
        );
    ...
    if (previousAvatarUrl && previousAvatarUrl !== publicUrlData.publicUrl) {
      setTimeout(async () => {
        try {
          ...
          console.warn(
            "🚨 PRODUCTION: Old avatar cleanup failed (non-critical):",
            cleanupError,
          );
        }
      }, 2000);
...
const deleteAvatarWithProductionSafety = async (
  publicUrl: string,
): Promise<{
  success: boolean;
  error?: string;
...
export default function Profile() {
    ...
    (
      ...
      try {
        if (isMountedRef.current) {
          ...
          console.warn(
            `🚨 PRODUCTION: Attempted ${stateName} update on unmounted component`,
          );
        }
      } catch (error) {
        console.error(
          `🚨 PRODUCTION: State update failed for ${stateName}:`,
          error,
        );
      }
    },
  ...
  const fetchPlayerStatisticsWithSafety = async (playerId: string) => {
    try {
      ...
      console.log(
        "📊 PRODUCTION: Starting stats fetch for:",
      ...
      console.log(
        "📊 PRODUCTION: Processing match data:",
        matchData?.length || 0,
        "matches",
      );

      // Safe stats calculation with error isolation
      const stats = await calculateStatsWithMaximumSafety(
        matchData || [],
        playerId,
      );
  ...
  const calculateStatsWithMaximumSafety = async (
    ...
    try {
      ...
      const baseRating = (() => {
        try {
          return profile?.glicko_rating
            ? parseFloat(profile.glicko_rating.toString())
            : 1500;
        } catch {
          return 1500;
        }
      ...
      const completedMatches = safeMatches
        .filter((match) => {
          try {
            return (
      ...
      for (const match of safeMatches) {
        try {
          ...
          console.warn(
            "🚨 PRODUCTION: Match processing error (non-critical):",
            matchError,
          );
          continue;
        }
      ...
      for (const match of completedMatches) {
        try {
          ...
          console.warn(
            "🚨 PRODUCTION: Win/loss calculation error (non-critical):",
            winLossError,
          );
          continue;
        }
      ...
      try {
        ...
        console.warn(
          "🚨 PRODUCTION: Performance calculation error (non-critical):",
          performanceError,
        );
      }
      ...
      console.error(
        "🚨 PRODUCTION: Stats calculation catastrophic failure:",
        error,
      );
  ...
  const handleAvatarUpload = async () => {
    try {
      ...
      const selection = await new Promise<"camera" | "library" | null>(
        (resolve) => {
          try {
            Alert.alert(
      ...
      const selectionResult = await selectImageWithProductionSafety(
        selection === "camera",
      );
      ...
      console.log(
        "📷 PRODUCTION: Starting upload with base64 data length:",
        selectionResult.base64.length,
      );
      safeSetState(
      ...
      console.error(
        "🚨 PRODUCTION: Avatar upload catastrophic failure:",
        error,
      );
      if (isMountedRef.current) {
        safeSetState(
          ...
          { text: "Cancel", style: "cancel" },
          {
            ...
            onPress: async () => {
              try {
                ...
                const deleteResult = await deleteAvatarWithProductionSafety(
                  profile.avatar_url!,
                );
  ...
  const renderBulletproofAvatar = () => (
    <View className="items-center mb-4">
      <TouchableOpacity
        ...
        style={{
          opacity: avatarState.uploading || avatarState.deleting ? 0.7 : 1,
        }}
      >
          ...
          <View className="w-32 h-32 rounded-full bg-primary items-center justify-center overflow-hidden">
            ...
            !avatarState.deleting ? (
              <Image
                ...
                onError={() => {
                  console.log(
                    "🚨 PRODUCTION: Avatar load failed, showing fallback",
                  );
                }}
              />
            ) : (
      ...
      {avatarState.error && (
        <View className="mt-2 p-3 bg-red-50 dark:bg-red-900/30 rounded-lg border border-red-200 dark:border-red-800 max-w-xs">
          ...
          >
            <Text className="text-xs text-red-600 dark:text-red-400 underline">
              Dismiss
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="px-6 pb-6">
              ...
              <View className="items-center">
                <Text className="text-base font-bold">
                  {playerStats.averageMatchDuration > 0
                    ? Math.round(
                        playerStats.averageMatchDuration / (1000 * 60),
                      ) + "m"
                    : "-"}
                </Text>
                <Text className="text-xs text-muted-foreground">
          ...
          <View className="flex-row justify-between items-center">
            <View className="flex-row items-center flex-1">
              <Ionicons
                name={
                  playerStats.recentPerformance === "improving"
                    ? "trending-up"
                    : playerStats.recentPerformance === "declining"
                      ? "trending-down"
                      : "remove"
                }
                size={18}
              ...
              <View className="flex-1">
                <Text className="text-sm text-muted-foreground">
                  ...
                  >
                    {" "}
                    {playerStats.streak > 0
                      ? `${playerStats.streak}W`
                      : playerStats.streak < 0
                        ? `${Math.abs(playerStats.streak)}L`
                        : "0"}
                  </Text>
                  {" • "}
                  <Text
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View className="px-6 pb-8 pt-2">
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-3">
              ...
              >
                <Ionicons
                  name="create-outline"
                  size={20}
                  color={colorScheme === "dark" ? "#a1a1aa" : "#777"}
                />
              </TouchableOpacity>
            </View>
            {safeRender(() =>
            ...
            {safeRender(() =>
              renderInfoCard("Number", profile?.number, "mail-outline"),
            )}
          </View>
