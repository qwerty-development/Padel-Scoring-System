...
const calculateEditPermissions = (match: EditableMatch, userId: string) => {
  ...
  const permissions = {
    ...
    canEditScores:
      isPast &&
      hoursSinceCompletion < 2 &&
      match.status !== MatchStatus.COMPLETED,
    isLocked:
      match.status === MatchStatus.COMPLETED && hoursSinceCompletion > 24,
    reason: "",
    warningMessage: "",
  };
  ...
  if (!isCreator) {
    ...
    permissions.warningMessage =
      "Match starting soon - limited editing available";
...
export default function EditMatchScreen() {
  ...
  const hasChanges = (): boolean => {
    ...
    if (
      JSON.stringify(originalPlayers.sort()) !==
      JSON.stringify(selectedFriends.sort())
    )
      return true;
        ...
        {editPermissions?.canEditPlayers ? (
          <View className="mb-6 p-4 rounded-xl bg-card border border-border/30">
            ...
            <View className="bg-muted/20 rounded-lg p-3 mb-4">
              <Text className="text-sm text-muted-foreground">
                Selected: You + {selectedFriends.length} friend
                {selectedFriends.length !== 1 ? "s" : ""}
              </Text>
            </View>

            <Button variant="outline" onPress={() => setShowPlayerModal(true)}>
              <Ionicons
                name="people-outline"
                size={18}
                style={{ marginRight: 8 }}
            ...
            {editPermissions.canEditVisibility && (
              <View className="mb-4">
                <View className="flex-row items-center justify-between mb-2">
                  ...
                  <View className="flex-row items-center">
                    <Text
                      className={`text-sm mr-3 ${!isPublicMatch ? "font-medium" : "text-muted-foreground"}`}
                    >
                      Private
                    </Text>
                    <TouchableOpacity
                    ...
                    <Text
                      className={`text-sm ml-3 ${isPublicMatch ? "font-medium" : "text-muted-foreground"}`}
                    >
                      Public
                    </Text>
                  </View>
