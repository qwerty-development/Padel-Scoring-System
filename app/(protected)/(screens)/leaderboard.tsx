...
function LeaderboardAvatar({
  ...
  if (shouldDisplayAvatarImage()) {
    return (
      <View
        className={`${config.containerClass} rounded-full ${calculateBackgroundColorClass()} items-center justify-center mr-4 overflow-hidden`}
      >
        <Image
        ...
        {imageLoading && (
          ...
          >
            <Text
              className={`${config.textClass} font-bold ${calculateTextColorClass()}`}
            >
  ...
  return (
    <View
      className={`${config.containerClass} rounded-full ${calculateBackgroundColorClass()} items-center justify-center mr-4`}
    >
      <Text
        className={`${config.textClass} font-bold ${calculateTextColorClass()}`}
      >
...
export default function Leaderboard() {
  ...
  const renderUserRankingCard = (user: UserRanking, index: number) => {
    ...
    const renderRankingIndicator = () => {
      ...
      const medalConfig =
        medalConfigurations[calculatedRank as keyof typeof medalConfigurations];
    ...
    return (
      ...
      >
        ...
        <View className="mr-3 w-8">{renderRankingIndicator()}</View>
  ...
  const renderEmptyState = () => (
    ...
    >
      ...
      <Text className="text-lg font-medium mt-2 mb-2 text-foreground">
        {viewType === "friends" ? "No friends found" : "No players found"}
      </Text>
  ...
  const currentRankings =
    viewType === "global" ? globalRankings : friendsRankings;
  const hasMoreUsers =
    viewType === "global" ? hasMoreGlobalUsers : hasMoreFriendUsers;
      ...
      <View className="flex-row border-b border-border bg-background">
        ...
        >
          <View className="flex-row justify-center items-center">
            ...
            />
            <Text
              className={`text-center font-medium ${viewType === "global" ? "text-primary" : "text-muted-foreground"}`}
            >
              Global
            </Text>
          </View>
        ...
        >
          <View className="flex-row justify-center items-center">
            ...
            />
            <Text
              className={`text-center font-medium ${viewType === "friends" ? "text-primary" : "text-muted-foreground"}`}
            >
              Friends
            </Text>
          </View>
          ...
          <>
            ...
            {currentRankings.length > 0
              ? currentRankings.map((user, index) =>
                  renderUserRankingCard(user, index),
                )
              : renderEmptyState()}
