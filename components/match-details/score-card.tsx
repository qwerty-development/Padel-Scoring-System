      ...
      <View className="mb-2">
        ...
        <View className="flex-row">
          ...
          <View className="flex-row flex-1 justify-between px-2">
            <Text
              className={`text-lg font-semibold ${team1Won ? "text-primary" : ""}`}
            >
              {team1Score}
            </Text>
            <Text
              className={`text-lg font-semibold ${team2Won ? "text-blue-500" : ""}`}
            >
              {team2Score}
            </Text>
          </View>
  ...
  return (
    <View className="bg-card rounded-xl p-6 mb-6">
      ...
      {userTeam && (winnerTeam === 1 || winnerTeam === 2) && (
        <View
          className={`absolute top-0 right-0 left-0 rounded-t-xl items-center justify-center py-1 ${
      ...
      {winnerTeam && (
        <View className="absolute top-12 left-1/2 -translate-x-6">
          <View
            className={`bg-${winnerTeam === 1 ? "primary" : "blue-500"}/10 p-2 rounded-full`}
          >
            <Ionicons
              name="trophy"
              size={24}
              color={winnerTeam === 1 ? "#2148ce" : "#3b82f6"}
            />
          </View>
        </View>
      )}
